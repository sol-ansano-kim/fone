...
class ParamTest(unittest.TestCase):
    ...
    def test_bool(self):
        b1 = self.param.FnCoreParamBool()
        self.assertIsNotNone(b1)
        self.assertEqual(b1.type(), self.param.FnCoreParamTypeBool)
        ...
        b3 = self.param.FnCoreParamBool(default=True)
        self.assertEqual(b3.default(), True)
        self.assertEqual(b3.get(), True)

        with self.assertRaises(self.exceptions.FnErrInvalidParamValueError):
            b1.set(1)
        with self.assertRaises(self.exceptions.FnErrInvalidParamValueError):
            b1.set(1.23)
        with self.assertRaises(self.exceptions.FnErrInvalidParamValueError):
    ...
    def test_int(self):
        i1 = self.param.FnCoreParamInt()
        self.assertIsNotNone(i1)
        self.assertEqual(i1.type(), self.param.FnCoreParamTypeInt)
        ...
        i3 = self.param.FnCoreParamInt(default=789)
        ...
        i4 = self.param.FnCoreParamInt(default=0, min=0, max=100)
    ...
    def test_float(self):
        f1 = self.param.FnCoreParamFloat()
        self.assertIsNotNone(f1)
        self.assertEqual(f1.type(), self.param.FnCoreParamTypeFloat)
        ...
        f3 = self.param.FnCoreParamFloat(default=789.0)
        ...
        f4 = self.param.FnCoreParamFloat(default=0.0, min=0.0, max=100.0)
    ...
    def test_str(self):
        s1 = self.param.FnCoreParamStr()
        self.assertIsNotNone(s1)
        self.assertEqual(s1.type(), self.param.FnCoreParamTypeStr)
        ...
        s3 = self.param.FnCoreParamStr(default="ghi")
        ...
        s4 = self.param.FnCoreParamStr(default="a", valueList=["a", "b", "c"], enforceValueList=True)
        ...
        with self.assertRaises(self.exceptions.FnErrInvalidParamValueError):
            self.param.FnCoreParamStr(default="A", valueList=["a", "b", "c"], enforceValueList=True)

        s5 = self.param.FnCoreParamStr(default="a", valueList=["a", "b", "c"], enforceValueList=False)
    ...
    def test_params(self):
        params = self.param.FnCoreParams({})
        self.assertIsNotNone(params)
        self.assertEqual(params.keys(), [])
        test = {
            "bool": self.param.FnCoreParamBool(),
            "int": self.param.FnCoreParamInt(),
            "float": self.param.FnCoreParamFloat(),
            "str": self.param.FnCoreParamStr()
        }

        params = self.param.FnCoreParams(test)
